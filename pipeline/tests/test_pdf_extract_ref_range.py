...
def test_vong_quoc_chu_glued_mchc303_rdw114():
    """VÒNG QUỐC CHỦ: bold/underline Ghi chú → MCHC303 / RDW11.4; không lấy 320/360."""
    text = """
Họ tên: VÒNG QUỐC CHỦ
Huyết học Công thức máu
Leukocytes (WBC) 6.85 ( 4.01 - 11.42 ) G/L
Erythrocytes (RBC) 4.52 ( 4.01 - 5.79 ) T/L
Hemoglobin (Hb) 130 ( 115 - 150 ) g/L
Hematocrit (Hct) 0.40 ( 0.34 - 0.49 ) L/L
MCV 88.0 ( 80.0 - 99.0 ) fL
MCH 28.0 ( 27.0 - 33.0 ) pg
MCHC303 ( 320 - 360 ) g/L
RDW11.4 ( 11.5 - 14.5 ) %
...
"""
    labs = normalize_for_web(parse_labs(text))
    assert (labs.get("MCHC") or {}).get("value_web") == "303"
    assert (labs.get("RDW") or {}).get("value_web") == "11.4"
    # Must NOT pick reference bounds
    assert (labs.get("MCHC") or {}).get("value_web") not in {"320", "360"}
    assert (labs.get("RDW") or {}).get("value_web") not in {"11.5", "14.5"}
    from medinet_api import labs_to_form_payload

    payload = labs_to_form_payload(labs, phieukham_id=1, gioi_tinh="Nam")
    assert payload.get("XNM_MCHC") == 303
    assert payload.get("XNM_RDW") == 11.4
    # No Urea on PDF → optional, must not invent
    assert "SinhHoaMau_Ure" not in payload or payload.get("SinhHoaMau_Ure") in (None, "")
...
if __name__ == "__main__":
    ...
    test_vong_quoc_chu_glued_mchc303_rdw114()
